...
select v.fecha_compra,mo.modelo,ma.marca,g.nombre,v.fecha_compra,v.matricula,c.nombre, kilometros, a.nombre, v.numero_poliza 
from vehiculos v
inner join modelos mo on v.id_modelo = mo.id 
inner join marcas ma on mo.id_marca = ma.id
inner join grupos g on ma.id_grupo = g.id
inner join colores c on v.id_color = c.id
inner join aseguradoras a on v.id_aseguradora = a.id 